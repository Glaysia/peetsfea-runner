...
class TestPlan03Workflow(unittest.TestCase):
    def test_remote_script_contains_required_environment_setup(self) -> None:
        ...
        self.assertIn('export TMP="${TMP:-$PEETS_DISK_TMPDIR}"', content)
        ...
        self.assertIn("    os.environ['TMP'] = str(tmpdir)", content)
        self.assertIn("    os.environ['TEMP'] = str(tmpdir)", content)
        self.assertIn("os.environ['TMPDIR'] = str(tmpdir)", content)
        self.assertIn("for env_key in ('ANSYS_WORK_DIR', 'TMP', 'TEMP', 'TMPDIR'):", content)
        ...
        self.assertIn("Bound AEDT session does not support analyze", content)
        ...
        self.assertIn("_project, _design, active_design_name = activate_project_design(", content)
        self.assertIn("if not hasattr(app, 'analyze'):", content)
        ...
        self.assertIn("grpc_port = int(reuse_grpc_port_value) if reuse_session else get_available_port()", content)
        ...
        self.assertIn("solve_project_name, solve_project = open_existing_project(solve_desktop, project_file)", content)
        self.assertIn("hfss_design_names, discovered_designs = discover_hfss_design_names(solve_project, project_file)", content)
        self.assertIn("solve_design_identifier = resolve_design_identifier(solve_project, hfss_design_names[0])", content)
        ...
        self.assertIn("setup_name = resolve_first_setup_name(solve_hfss, project_file, solve_design_name)", content)
        self.assertIn("solved = solve_hfss.analyze(", content)
        self.assertIn("solve_in_batch=False,", content)
        self.assertIn("blocking=True,", content)
        ...
        self.assertIn("extract_design_reports(solve_desktop, workdir, project_file, grpc_port)", content)
    ...
    def test_remote_dispatch_script_uses_noninteractive_srun_without_screen(self) -> None:
        ...
        self.assertIn('srun -D "/tmp/$USER/peetsfea-runner/_runtime" -p cpu2 -N 1 -n 1 -c 16 --mem=960G --time=05:00:00 --exclude=n108,n109', content)
    ...
    def test_pull_worker_payload_uses_single_container_sshfs_slots(self) -> None:
        ...
        self.assertIn("write_container_bootstrap_script() {", content)
        self.assertEqual(content.count('export PEETS_WORKSPACE_REMOTE="peets@172.16.165.146:/home/peets/mnt/8tb/peetsfea-runner"'), 2)
        self.assertEqual(content.count('export PEETS_WORKSPACE_MOUNT_ROOT="/workspace"'), 2)
        self.assertIn('test -f /etc/peetsfea_ssh/id_control', content)
        self.assertIn("IdentityFile=/etc/peetsfea_ssh/id_control", content)
        self.assertIn(
            'sshfs -o reconnect,follow_symlinks,ServerAliveInterval=15,ServerAliveCountMax=3,idmap=user,uid=0,gid=0,umask=000',
            content,
        )
        ...
        self.assertGreaterEqual(content.count("enroot start --root --rw"), 2)
        self.assertIn('--mount "/dev/fuse:/dev/fuse"', content)
        self.assertIn('--mount "$ssh_mount_dir:/etc/peetsfea_ssh"', content)
        self.assertNotIn('--mount "$ssh_mount_dir:/root/.ssh:ro"', content)
        self.assertIn('local -a container_base_mounts=', content)
        self.assertIn('local -a container_runtime_mounts=("${container_base_mounts[@]}" --mount "$ssh_mount_dir:/etc/peetsfea_ssh")', content)
        self.assertIn('enroot start --root --rw "${container_base_mounts[@]}" "$container_name" "${container_prep_cmd[@]}"', content)
        self.assertIn('enroot start --root --rw "${container_runtime_mounts[@]}" "$container_name" /bin/bash /work/container_bootstrap.sh', content)
        self.assertIn("copy_runtime_debug_back() {", content)
        self.assertIn("copy_runtime_debug_back", content)
        self.assertIn('"$workdir"/container*.stderr', content)
        self.assertIn('"$workdir"/enroot.create.stderr', content)
        self.assertNotIn("container_ssh/id_control", content)
        ...
        self.assertIn('container_case_dir="/work/slots/${case_name}"', content)
        self.assertIn('cd "\\$container_case_dir"', content)
        self.assertIn("cleanup_slot_processes() {", content)
        self.assertIn('cleanup_log="\\$container_case_dir/process_cleanup.log"', content)
        self.assertIn('target_disk="\\$case_disk_root"', content)
        self.assertIn('target_tmpfs="\\$case_ram_root"', content)
        self.assertIn("*ansysedt.exe*|*ansyscl*|*HFSSCOMENGINE.EXE*|*HFSSCOMENGINE*|*hfsscomengine*)", content)
        self.assertIn('kill -9 \\$pids', content)
        self.assertIn("bash ./remote_job.sh || slot_rc=\\$?", content)
        self.assertIn("cleanup_slot_processes || true", content)
        self.assertIn('exit "\\$slot_rc"', content)
        self.assertNotIn("pkill ansysedt", content)
        self.assertNotIn("killall ansysedt", content)
        self.assertLess(content.index("bash ./remote_job.sh || slot_rc=\\$?"), content.index("cleanup_slot_processes || true"))
        self.assertLess(content.index("cleanup_slot_processes || true"), content.index('cp -a "\\$container_case_dir/." "$output_dir/"'))
        self.assertLess(content.index('cp -a "\\$container_case_dir/." "$output_dir/"'), content.index('exit "\\$slot_rc"'))
        self.assertNotIn('cd "$case_dir"', content)
        self.assertNotIn('mkdir -p "$output_dir"', content)
        self.assertIn("artifact_bytes=0", content)
        ...
        self.assertIn('mkdir -p /etc/peetsfea_ssh /workspace /work/slots', content)
        self.assertLess(
            content.index('enroot start --root --rw "${container_base_mounts[@]}" "$container_name" "${container_prep_cmd[@]}"'),
            content.index('enroot start --root --rw "${container_runtime_mounts[@]}" "$container_name" /bin/bash /work/container_bootstrap.sh'),
        )
    ...
    def test_pull_remote_sbatch_only_stages_scripts(self) -> None:
        ...
        self.assertNotIn('EXEC_ROOT="$HOME/aedt_runs/_submit"', content)
    ...
    def test_enroot_worker_payload_uses_cd_then_mount_contract(self) -> None:
        ...
        self.assertIn('enroot start --root --rw --mount "$REMOTE_HOST_ANSYS_ROOT:/mnt/AnsysEM" --mount "$REMOTE_HOST_ANSYS_BASE:/ansys_inc/v252" --mount "$REMOTE_HOST_ANSYS_BASE/licensingclient:/mnt/licensingclient" --mount "$case_dir:/work" --mount "$JOB_TMPFS_ROOT:/job_tmpfs" --mount "$JOB_DISK_ROOT:/job_disk"', content)
