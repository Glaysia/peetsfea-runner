...
class TestPlan03Workflow(unittest.TestCase):
    def test_remote_script_contains_required_environment_setup(self) -> None:
        ...
        self.assertIn("cmd = [ANSYS_EXECUTABLE, '-ng', '-grpcsrv', str(port)]", content)
        ...
        self.assertIn("process = subprocess.Popen(cmd, cwd='/tmp', stdout=stdout_handle, stderr=stderr_handle, env=launch_env)", content)
        self.assertIn("from contextlib import contextmanager", content)
        self.assertIn("from typing import Any", content)
        self.assertIn("from ansys.aedt.core import Desktop, Hfss, Maxwell3d", content)
        ...
        self.assertIn("solve_targets = discover_batch_solve_targets(project_file, hfss_design_names)", content)
        ...
        self.assertIn("for candidate_project_ref in (project_name, str(project_file.resolve())):", content)
        ...
        self.assertIn("app = attach_design_app(desktop, project_name, project_file, design_identifier, design_name, 'HFSS', grpc_port)", content)
        ...
        self.assertIn("def run_batch_solve(project_file: Path, solve_targets: list[str], *, cores: int, tasks: int) -> None:", content)
        self.assertIn("queue_file = Path(str(project_file) + '.q')", content)
        self.assertIn("queue_complete = Path(str(project_file) + '.q.complete')", content)
        self.assertIn("queue_completed = Path(str(project_file) + '.q.completed')", content)
        self.assertIn("for index, solve_target in enumerate(solve_targets, start=1):", content)
        self.assertIn("batch_command = [", content)
        self.assertIn("'-BatchSolve',", content)
        self.assertIn("f'list=localhost:{tasks}:{cores}:90%:1',", content)
        self.assertIn("solve_target,", content)
        self.assertIn("batch_log_path = project_file.parent / 'batch.log'", content)
        self.assertIn("batch_log_handle.write(f'=== solve_target[{index}]={solve_target} ===\\n')", content)
        self.assertIn("stdout=batch_log_handle,", content)
        self.assertIn("stderr=subprocess.STDOUT,", content)
        self.assertIn("wait_for_path(queue_file, timeout_seconds=120, process=process)", content)
        self.assertIn(
            "wait_for_any_path((queue_complete, queue_completed), timeout_seconds=3600, process=process)",
            content,
        )
        self.assertIn("process_return_code = process.wait(timeout=120)", content)
        self.assertIn("def reopen_solved_project(project_file: Path, grpc_port: int) -> Desktop:", content)
        ...
        self.assertIn("def solve_project_in_batch(project_file: Path, grpc_port: int, *, cores: int, tasks: int) -> None:", content)
        self.assertIn(
        ...
        self.assertIn("hfss_design_name = str(hfss_design_identifier).rsplit(';', 1)[-1].strip() or hfss_design_identifier", content)
        self.assertIn("f'[INFO] batch solve prepared project={project_name!r} '", content)
        self.assertIn("f'design={hfss_design_name!r} discovered_designs={discovered_designs!r} solve_targets={solve_targets!r}'", content)
        self.assertIn("release_desktop_session(discovery_desktop, close_projects=True, close_desktop=False)", content)
        self.assertIn("run_batch_solve(project_file, solve_targets, cores=cores, tasks=tasks)", content)
        self.assertIn("solve_project_in_batch(project_file, grpc_port, cores=cores, tasks=tasks)", content)
        self.assertIn("reopened_desktop = reopen_solved_project(project_file, grpc_port)", content)
        self.assertIn("reopened_project = reopened_desktop.odesktop.GetActiveProject()", content)
        self.assertIn("reopened_project.Save()", content)
        self.assertIn("extract_output_variables_csv(reopened_desktop, workdir, project_file, grpc_port)", content)
        ...
        self.assertIn("def merge_design_row(target_row: dict[str, object], design_name: str, design_row: dict[str, object], *, include_unprefixed: bool) -> None:", content)
        self.assertIn("def extract_output_variables_csv(desktop: Desktop, workdir: Path, project_file: Path, grpc_port: int) -> None:", content)
        ...
        self.assertIn("def export_all_reports(app: Any, workdir: Path) -> dict[str, Path]:", content)
        self.assertIn("reports_dir = workdir / 'all_reports' / design_key", content)
        ...
        self.assertNotIn("merge_report_rows(row, read_report_rows(report_path), report_name, bare_first_row=False)", content)
        ...
        self.assertNotIn("solve_hfss.solve_in_batch(file_name='./project.aedt', cores=cores, tasks=tasks)", content)
    ...
    def test_worker_payload_can_refill_pool_with_prefetched_cases(self) -> None:
        ...
        self.assertIn("sync_case_artifacts_back \"$case_dir\"", content)
    ...
    def test_enroot_worker_payload_uses_cd_then_mount_contract(self) -> None:
        ...
        self.assertIn('enroot start --root --rw --mount "$HOME:$HOME" --mount "$REMOTE_HOST_ANSYS_ROOT:/mnt/AnsysEM" --mount "$case_dir:/work"', content)
        ...
        self.assertIn('mkdir -p "$REMOTE_JOB_DIR/$case_dir"', content)
    ...
    def test_enroot_remote_job_script_uses_image_python_and_mounted_ansys(self) -> None:
        ...
        self.assertIn("release_desktop_session(discovery_desktop, close_projects=True, close_desktop=False)", content)
        self.assertNotIn("solve_hfss.solve_in_batch(file_name='./project.aedt', cores=cores, tasks=tasks)", content)
        self.assertNotIn("solve_hfss = attach_hfss_to_open_project(", content)
        self.assertIn("app = build_design_app(project_reference, requested_design_name, design_factory, grpc_port)", content)
        self.assertIn("design = project.SetActiveDesign(candidate)", content)
