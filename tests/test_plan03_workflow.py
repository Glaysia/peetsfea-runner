...
class TestPlan03Workflow(unittest.TestCase):
    def test_remote_script_contains_required_environment_setup(self) -> None:
        ...
        self.assertIn("\"$IMAGE_PYTHON\" run_sim.py", content)
        ...
        self.assertIn("def reports_dir_for_design(workdir: Path, hfss: Hfss) -> Path:", content)
        ...
        self.assertIn('"$IMAGE_PYTHON" -m uv --version', content)
    ...
    def test_remote_script_merges_nominal_variations_into_csv_row_before_outputs(self) -> None:
        ...
        self.assertIn("reports_dir = reports_dir_for_design(workdir, hfss)", content)
