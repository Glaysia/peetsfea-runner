...
## 6. 로컬 enroot = 클러스터 enroot (로컬 검증 활용)
1. 로컬 PC와 계산노드(SLURM 잡) **둘 다 enroot 3.5.0**로 동일하다(`/usr/bin/enroot`).
   따라서 **로컬 enroot 컨테이너에서 되는 컨테이너 동작은 잡 안에서도 거의 그대로 된다.**
2. 컨테이너 레벨 작업(이미지/sqsh, 마운트, 컨테이너 내부 실행, edtmgr·ansysedt 기동 등)은
   **먼저 로컬 enroot에서 빠르게 재현·검증**하고, 그 다음 잡으로 올린다.
3. 단, 컨테이너 밖 요소는 로컬과 다를 수 있으니 그대로 옮기지 않는다:
   SLURM 할당/스케줄링, 노드별 마운트(`/opt/.../AnsysEM`, 공유 `$HOME`), 라이선스 서버·네트워크
   토폴로지(역터널/ sshfs). 이런 건 잡에서 별도 확인한다.
