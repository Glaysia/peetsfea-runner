...
- 상태 DB 기본 경로는 `peetsfea_runner.duckdb`다.
...
- canary/validation은 live DB나 live output과 섞지 않는 별도 경로를 쓰는 것이 원칙
...
config = PipelineConfig(
    ...
    metadata_db_path=str(repo / "tmp" / "validation" / "state.duckdb"),
    accounts_registry=(
...
4. 결과는 `output/<lane>`와 `peetsfea_runner.duckdb`에 반영된다.
...
- validation run은 live output과 live DB를 재사용하지 않는 편이 안전하다.
- `output_variables.csv` gate의 구체 규칙은 active [`PLANS/`](../../PLANS/)를 본다.

즉 처음 붙여볼 때는 service 재기동보다, 별도 DB와 별도 output 경로를 둔 1회성 validation run부터 확인하는 편이 맞다.
