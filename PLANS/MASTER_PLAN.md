...
## 0. 한 줄 요약
단일 계정에서 정상상태 동안 **동시 ~100개**(컨테이너 9개, 각 컨테이너 최대 16개 ansysedt를
부하에 따라 유동 가동) AEDT 시뮬레이션을 끊김 없이 유지하고, 입력 큐(랜덤 샘플링) → 시뮬 →
결과 DB/대시보드 → 아카이브까지 하나로 묶는 오케스트레이터. ansysedt를 껐다 켜는 비용이 크므로
`edtmgr`가 warm 상태와 EDT 라이선스 점유를 유지하고, **로드 밸런서**가 부하 곡선에 맞춰 시작
시점과 동시 가동 수를 조절한다.
...
| ansysedt | 컨테이너당 **warm 하한 11, 최대 16** | edtmgr가 항상 ≥11 warm 보유; 동시 실행은 부하 따라 유동 |
| edtmgr | 컨테이너당 ansysedt 1개당 1개 | **runner에 위치(Q1)**, warm ≥11, ≤16 |
| 동시 시뮬레이션(실행) | 계정당 목표 **~100개** | 9 컨테이너 합. 컨테이너별 **동시 실행 ~7–16 불균형 허용** |
| 잡 수명 | **10시간** | 만료 시 진행 중 ~100개 **그냥 폐기**(Q8) |

- **유연 토폴로지(마진 허용):** 9×균등이 아니라 **컨테이너 9개 / 각 warm 하한 11·최대 16 / 합 ~100 동시 실행**이 기준.
  - **warm 보유(held)와 동시 실행(running)을 구분**한다. edtmgr는 **컨테이너당 항상 ≥11 warm 보유**
    (그 순간 7개만 시뮬을 돌리고 있어도 11개는 잡고 대기). **동시 실행** 수는 부하 따라 ~7–16 유동.
  - 부하 많은 컨테이너는 운 나쁘면 동시 실행 ~7개, 가벼운 컨테이너는 ~13–16처럼 **불균형 허용**
    (합이 ~100 근처면 OK). warm은 어느 경우든 11 미만으로 안 내려간다.
  - 9×11=99가 **상시 warm 베이스**(목표 ~100과 정합). 실제 동시 실행/추가 가동은 로드 밸런서가 결정(§2.3).
  - 클러스터 검증(부록 A·검증): 컨테이너당 **10개 동시 warm** 확인, 노드 1TB RAM·라이선스 550으로
    16까지 여유.
...
```
   ...
   │  │   ...                                                        │
   │  │ slot_N (≤16): edtmgr ── ansysedt(grpc)  ← 부하 따라 7~16 가동 │
   │  └───────────────────────────┘                                  │
...
- **구성:** 컨테이너당 **최대 16개**(ansysedt 1개당 edtmgr 1개). 각 edtmgr는 자신의
  ansysedt를 `-ng -grpcsrv <port>` 로 띄우고 관리 세션을 `close_on_exit=False` 로 붙여 둔다.
  edtmgr는 **컨테이너당 최소 11개를 항상 warm으로 유지**한다(그 순간 7개만 시뮬을 돌리고 있어도
  11개는 잡고 대기 → 부하 완화 시 슬로 스타트 없이 즉시 11까지 가동). **11을 넘어 12~16으로 늘릴
  때**만 ansysedt를 추가 기동(재할당)하며, 그 **기동 지연은 무방**. warm은 11 미만으로 내려가지 않는다.
  동시 실행(대여) 수는 로드 밸런서가 부하 기반으로 7~16에서 결정(§2.3 ramp-up).
...
- 각 슬롯에서 fixed toml을 **순차 연속** 실행(끝나면 즉시 다음). 합계 목표 **~100개 동시**
  (컨테이너별 ~7–16 유동, §2.3).
...
### 2.3 로드 밸런서 (Q3 반영) — ramp-up + 유연 토폴로지
- **문제:** 시뮬을 한꺼번에 켜면 패스가 커지는(자원 최대) 구간이 동시에 겹쳐 노드 부하가 스파이크 →
  활용률·스루풋 저하. 한 시뮬의 자원 사용은 시간에 따라 증가한다(초기 근사 `t^1.5`, 패스 커질수록 ↑) —
  즉 **시작 직후엔 가볍고 끝 무렵(큰 패스)엔 무겁다.**
- **핵심 통찰(ramp-up):** 새로 켜는 시뮬의 **즉각 부하는 작다.** 그러므로
  **현재 총 부하에 여유가 있을 때 새 시뮬을 "싸게" 끼워 넣고**, 기존 시뮬들이 무거운 패스로
  들어가 부하가 커지면 신규 시작을 늦춘다. 이렇게 하면 시작 시점이 자연히 분산(stagger)되어
  무거운 구간이 시간축에서 흩어진다.
  - **콜드 스타트:** 처음부터 ~100개를 켜지 않고 **적게 시작 → 부하 여유에 따라 점진 증설**.
    새로 켜지는 시뮬은 자기 초기 구간(가벼움)에 들어오므로, 기존이 무거워지기 전에 흡수된다.
- **신호:** 노드/컨테이너의 **CPU·메모리** 사용률(주기 샘플링; psutil + `/proc`).
- **제어 루프(검증된 기법):**
  - 측정 부하를 목표치(setpoint: CPU/메모리 watermark)로 수렴시키는 **피드백 제어(PID 또는 EWMA)**.
  - **admission control:** 슬롯이 비고(시뮬 완료) **그리고** 측정 부하가 목표 이하이면 다음 fixed toml을
    디스패치(필요 시 edtmgr가 ansysedt를 추가 기동). 과부하면 **백오프(AIMD류)** 로 신규 시작 보류.
  - **시작 스태거:** 동시에 우르르 켜지지 않게 시작 간 최소 간격 + 부하 게이팅.
  - 매시간 단위 재최적화(누적 부하 프로파일로 목표/게인 조정).
- **유연 토폴로지(탄력적 슬롯, 마진 허용):**
  - 컨테이너 9개, 각 **warm 하한 11 · 최대 16**. 목표 합계 **~100개 동시 실행**(9×11=99가 상시 warm 베이스).
  - **warm 보유 ≥11 (항상)** vs **동시 실행 7–16 (부하 유동)** 을 구분한다. 7개만 돌더라도 11개는 warm으로
    잡고 있어, 부하가 풀리면 **슬로 스타트 없이 즉시 11까지** 가동. **12~16 버스트**만 ansysedt 추가
    기동이 필요하고 그 기동 지연은 무방.
  - **균등 분배를 강제하지 않는다.** 부하 많이 걸린 컨테이너는 운 나쁘면 동시 실행 ~7개만, 가벼운
    컨테이너는 ~13–16처럼 **불균형을 허용**(합이 ~100 근처면 OK). 엄격 균등은 복잡도만 키우고 활용률↓.
  - 실제 동시 실행 수는 LB가 부하 기반으로 컨테이너별 결정. 라이선스는 충분(부록: feature당 550)하므로
    제약은 노드 자원(CPU/메모리)이지 라이선스가 아니다. (warm 99개가 ~99 라이선스 상시 점유.)
...
- 흐름: 온전한 sweep toml 수신 → peetsfea가 **범위 검증(기준 이내인지)** → 통과 시
  **랜덤 샘플링으로 fixed toml × N** 생성 → **대기 큐** 적재(우선순위 레인) → 슬롯 가용 시 디스패치 → 시뮬 후 DB 반영.

### 2.6.1 큐 정책: baseline 탐색 + 우선순위 어댑티브 + 15% 탐색 플로어
- **주요 목표:** `examples/0.3.2_sweep.toml`이 정의하는 **전체 설계공간을 학습**한다.
  이 공간은 **자유변수 20차원, 그리드 곱 ≈ 8.25×10³⁰** → **열거 불가, 사실상 연속공간**으로 취급.
- **두 레인 모델 (가중 스케줄러):**
  - **baseline 레인 (전체공간 탐색):** 전체 `0.3.2_sweep.toml`을 **풀스페이스 랜덤 샘플링**해
    **~1000칸 버퍼를 상시 리필**한다. 깊이가 저수위로 떨어지면 다음 배치를 샘플링해 다시 채운다
    (`sample_fixed_candidates_from_toml_text`, **배치마다 seed 롤링** → 같은 1000개 반복 방지).
    슬롯이 **놀지 않게** 하는 상시 공급원이자 전역 커버리지 담당.
  - **우선순위 레인 (어댑티브):** 7875로 들어오는 건 전체공간의 **subset sweep toml**이다
    (어댑티브 러너가 더 조밀히 보고 싶은 영역; 분해능을 높인 좁은 range). 검증·샘플링 후
    **우선순위 레인 앞쪽**에 적재돼 먼저 소비된다.
- **15% 탐색 플로어 (anti-starvation):** 디스패처는 두 레인을 **~85:15로 교차 추출**한다
  (결정론적 인터리브, 예: 7건당 1건은 baseline). 우선순위 backlog가 쌓여도 **baseline은 15% 하드
  플로어**를 유지해 전역 커버리지가 무너지지 않는다. 우선순위 레인이 비면 **100% baseline**.
  → 단순 single-deque front-insert가 아니라 **2-레인 가중 스케줄러**로 구현(현 `edt_queue.TomlQueue`
  단순 FIFO를 우선순위/baseline 2-레인 + 비율 스케줄러로 확장; 디스패치 지점 `edt_dispatcher.py:95`).
- **중복:** 별도 dedup 없음. 8e30 연속공간이라 baseline 재추출 확률은 무시 가능하고, subset도
  분해능을 높여 충돌이 드물다 — 단순성 유지(필요해지면 결과 DB `point_hash`로 후속 도입).
- **재시작 영속성:** **baseline은 휘발**(seed로 언제든 재생성). **우선순위(7875) 요청만 파일로 영속**
  → 제어플레인 durable truth는 우선순위 레인(§2.7과 일관). 재시작 시 어댑티브 의도 보존.
...
- `localhost:8080` 대시보드는 이 DB를 **읽기 전용**으로 시각화/확인만. 시뮬에 영향 주는
  입력은 받지 않는다.
- **CSV 내보내기 엔드포인트(읽기 전용):** `curl`로 **현재까지의 누적 결과(입력 파라미터 + 출력
  변수 `k_ratio`/`Lrx_uH` 등)를 CSV로** 받는 GET 요청을 제공한다.
  예) `curl localhost:8080/results.csv` → 한 행 = 한 시뮬(입출력 파라미터 + 시간/패스/상태).
  쿼리 파라미터로 필터/범위 지정 가능(예: `?since=<iso>`, `?terminal_state=succeeded`, `?limit=N`).
  결과 DB(`single_simulation_store.py`)를 그대로 조회만 하며 시뮬에 영향 없음.
...
### Phase 2 — 계정 내 9잡/~100 동시 오케스트레이션
- 9잡 제출·생애주기 관리, 잡당 컨테이너 1개, 슬롯 풀 **컨테이너당 ≤16 / 합 ~100 동시**(유동), 10h 만료 시 폐기(Q8).
- 잡 단위 `/enroot/{USER}_{SJOB}` 생성/정리, 잡 재기동(드레인 없음).
- **수용:** 정상상태에서 동시 ~100개 유지가 관측됨(컨테이너별 ~7–16 유동, 합 ~100).
...
- sweep toml + N 수신 → 랜덤 샘플 fixed toml × N → 대기 큐 적재 → 시뮬 → DB 반영.
- **큐 정책(§2.6.1):** baseline 전체공간 ~1000 버퍼 상시 리필 + 7875 subset은 우선순위 레인,
  **85:15 가중 스케줄러**(baseline 15% 탐색 플로어). baseline 휘발 / 우선순위 영속.
- **수용:** ① baseline만으로 슬롯이 끊김 없이 채워지고, ② sweep 1건 투입 시 N개가 우선 처리되며,
  ③ 우선순위 backlog 중에도 baseline이 ~15% 유지됨이 데이터로 확인.

### Phase 5 — 결과 DB + 대시보드 `:8080`
- DuckDB 스키마 확장(load/timing/IO param/archive ref), 8080 read-only 대시보드
  + **CSV 내보내기 GET 엔드포인트**(`curl localhost:8080/results.csv`, 입출력 파라미터 누적).
- **수용:** 누적 데이터가 대시보드에서 조회되고, `curl`로 현재까지 결과 CSV를 받을 수 있으며, 시뮬에 영향 없음.
...
1. 토폴로지/구성 재정의: 단일 계정 / 9잡 / 잡당 컨테이너 1 / 컨테이너당 ansysedt·edtmgr 10 /
   컨테이너당 ≤16 / 동시 ~100(유동) / 잡 수명 10h. 현 `DEFAULT_SLURM_JOB_TIME_LIMIT="00:45:00"`
   (`peetsfea_runner/constants.py:6`) → 10h. 현 `runner.py:46,60-61`(slots_per_job,
   account_01 max_jobs) 정비.
2. **edtmgr(신규 모듈):** 컨테이너 내 최대 16개 관리 서버 + 대여 프로토콜 + 60/65분 타이밍 +
...
- 대기 큐 구현: **2-레인(우선순위 영속 파일 + baseline 인메모리 휘발) + 85:15 가중 스케줄러**로 확정(§2.6.1). 잔여는 인터리브 비율 상수·저수위 리필 임계 튜닝.
