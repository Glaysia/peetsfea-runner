...
- `01`: 재시작, `dispatch.mode`, 콜드 스타트 기준, sample canary, Go/No-Go
...
- durable truth는 `input_queue`, `output`, `.done`이다.
- restart는 콜드 스타트로 본다.
...
| 창 | 코드 반영 범위 | 상태 처리 | canary multiplier | Green 신호 | No-Go 조건 | start 후 확인 |
| --- | --- | --- | --- | --- | --- | --- |
| `00:00` | CSV integrity hotfix, canary schema gate, 문서/운영 경계 정리 | 콜드 스타트 | `1` | `CANARY_PASSED`, `FULL_ROLLOUT_READY`, `reason=ok` | `CANARY_FAILED`, `reason=output_variables_csv_schema_invalid`, `reason=output_variables_csv_missing`, `reason=materialized_output_missing` | `WORKER_LOOP_START`, `WORKER_LOOP_OK` 또는 `WORKER_LOOP_IDLE`, `SLURM_TRUTH_REFRESHED` |
| `02:00` | `worker_bundle_multiplier` 상승 | 콜드 스타트 | target `4`, fallback `2` | `00:00` green 유지, canary green, idle slot 감소 기대 | CSV gate red 재발, `failed_slots` 급증, throughput 개선 없음 | `WORKER_LOOP_OK`, `failed_slots` 급증 없음 |
| `04:00` | license-max 520 uniform slot scheduler 반영 | 콜드 스타트 | 직전 green 값 유지 | CSV gate green 유지, license poll/control green 유지 | `LICENSE_POLL_FAILED`, `LICENSE_CEILING_REACHED`, canary red, restart red | `WORKER_LOOP_OK`, license poll ok, slot budget 증가 |
| `06:00` | final hardening, threshold/logging, 최종 값 고정 | 콜드 스타트 | `4` 또는 안정 fallback 값 | CSV regression 없음, canary green, restart green | `WORKER_LOOP_ERROR`, CSV red 재발, restart red | `WORKER_LOOP_OK/IDLE`, `WORKER_LOOP_ERROR` 부재 |

주의사항은 아래와 같다.

- 현재 구현 기준에서는 DB를 운영 진실로 사용하지 않는다.
- 입력/출력 파일시스템과 `.done` 규칙이 상태 판단의 기준이다.
