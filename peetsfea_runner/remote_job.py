...
def _build_worker_payload_script_content(
    *,
    ...
    payload_lines = [
        ...
        "  fi",
        "  rm -f \"$PEETS_TUNNEL_SOCKET\" >/dev/null 2>&1 || true",
        "}",
...
def _build_pull_worker_payload_script_content(
    *,
    ...
    runtime_root = _remote_runtime_root_shell_path(config=config)
    workspace_user = str(getattr(config, "pull_workspace_user", "peets")).strip() or "peets"
    workspace_host = str(getattr(config, "pull_workspace_host", "172.16.165.146")).strip() or "172.16.165.146"
    workspace_path = str(getattr(config, "pull_workspace_path", "/home/peets/mnt/8tb/peetsfea-runner")).strip()
    workspace_mount_root = str(getattr(config, "pull_workspace_mount_root", "/workspace")).strip() or "/workspace"
    workspace_remote = f"{workspace_user}@{workspace_host}:{workspace_path}"
    new_payload_lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "export PATH=/usr/bin:/bin:/usr/sbin:/sbin:${PATH:-}",
        ...
        "PEETS_CONTROL_SSH_IDENTITY=\"${PEETS_CONTROL_SSH_IDENTITY:-}\"",
        f"PEETS_REMOTE_CONTAINER_RUNTIME={shlex.quote(container_runtime)}",
        "REMOTE_CONTAINER_IMAGE=" + _double_quoted_shell_value(_remote_path_for_shell(config=config, path=container_image)),
        "REMOTE_HOST_ANSYS_ROOT=" + _double_quoted_shell_value(_remote_path_for_shell(config=config, path=host_ansys_root)),
        "REMOTE_HOST_ANSYS_BASE=" + _double_quoted_shell_value(_remote_path_for_shell(config=config, path=host_ansys_base)),
        "export PEETS_WORKSPACE_REMOTE=" + _double_quoted_shell_value(workspace_remote),
        "export PEETS_WORKSPACE_MOUNT_ROOT=" + _double_quoted_shell_value(workspace_mount_root),
        ...
        "  local -a active_pids=()",
        "  for pid in \"${case_pids[@]}\"; do",
        "    if kill -0 \"$pid\" >/dev/null 2>&1; then active_pids+=(\"$pid\"); fi",
        "  done",
        "  case_pids=(\"${active_pids[@]}\")",
        "  printf '%s\\n' \"${#case_pids[@]}\"",
        "}",
        ...
        "  used_kb=$(( total_kb - avail_kb ))",
        "  if [ \"$used_kb\" -lt 0 ]; then used_kb=0; fi",
        "  if [ \"$total_kb\" -gt 0 ]; then pressure_pct=$(( (used_kb * 100) / total_kb )); else pressure_pct=0; fi",
        "  printf '%s %s %s %s\\n' \"$(( total_kb / 1024 ))\" \"$(( avail_kb / 1024 ))\" \"$(( used_kb / 1024 ))\" \"$pressure_pct\"",
        "}",
        "memory_gate_open=1",
        "last_target_slots=''",
        ...
        "  $(memory_stats)",
        "EOF",
        "  if [ \"${memory_pressure_pct:-0}\" -ge \"$PEETS_SLOT_MEMORY_PRESSURE_HIGH_WATERMARK\" ]; then memory_gate_open=0; elif [ \"${memory_pressure_pct:-0}\" -le \"$PEETS_SLOT_MEMORY_PRESSURE_RESUME_WATERMARK\" ]; then memory_gate_open=1; fi",
        "  if [ \"$memory_gate_open\" -eq 1 ]; then",
        "    if [ \"$active_now\" -lt \"$PEETS_SLOT_MIN_CONCURRENCY\" ]; then target_slots=\"$PEETS_SLOT_MIN_CONCURRENCY\"; else target_slots=\"$max_parallel\"; fi",
        "  else",
        "    target_slots=\"$active_now\"",
        "  fi",
        "  if [ \"$target_slots\" -gt \"$max_parallel\" ]; then target_slots=\"$max_parallel\"; fi",
        ...
        "  endpoint=\"$1\"; payload=\"${2:-{}}\"; method=\"${3:-POST}\"; control_python=\"$(control_python_bin)\"",
        "  if [ -z \"$control_python\" ]; then return 1; fi",
        ...
        "req = urllib.request.Request(os.environ['CONTROL_API_URL'] + os.environ['CONTROL_ENDPOINT'], data=os.environ['CONTROL_PAYLOAD'].encode('utf-8') if os.environ['CONTROL_METHOD'] != 'GET' else None, headers={'Content-Type': 'application/json'}, method=os.environ['CONTROL_METHOD'])",
        "try:",
        ...
        "  endpoint=\"$1\"; output_path=\"$2\"; control_python=\"$(control_python_bin)\"",
        "  if [ -z \"$control_python\" ]; then return 1; fi",
        ...
        "}",
        "start_control_tunnel() {",
        "  if [ -z \"$PEETS_CONTROL_RETURN_HOST\" ] || [ -z \"$PEETS_CONTROL_RETURN_USER\" ]; then return 1; fi",
        "  local -a ssh_args=(-F /dev/null -p \"$PEETS_CONTROL_RETURN_PORT\" -o BatchMode=yes -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null)",
        "  if [ -n \"$PEETS_CONTROL_SSH_IDENTITY\" ]; then ssh_args+=(-o IdentitiesOnly=yes -i \"$PEETS_CONTROL_SSH_IDENTITY\"); fi",
        "  ssh \"${ssh_args[@]}\" -M -S \"$PEETS_TUNNEL_SOCKET\" -fnNT \\",
        "    -L 127.0.0.1:${PEETS_CONTROL_LOCAL_PORT}:127.0.0.1:${PEETS_CONTROL_PORT} \\",
        "    \"${PEETS_CONTROL_RETURN_USER}@${PEETS_CONTROL_RETURN_HOST}\"",
        "}",
        ...
        "  rm -f \"$PEETS_TUNNEL_SOCKET\" >/dev/null 2>&1 || true",
        "}",
        "emit_control_register() { control_plane_request_json /internal/workers/register \"{\\\"run_id\\\":\\\"${PEETS_CONTROL_RUN_ID}\\\",\\\"worker_id\\\":\\\"${PEETS_CONTROL_WORKER_ID}\\\",\\\"tunnel_session_id\\\":\\\"${PEETS_TUNNEL_SESSION_ID}\\\",\\\"slurm_job_id\\\":\\\"${SLURM_JOB_ID:-}\\\",\\\"observed_node\\\":\\\"${SLURMD_NODENAME:-${HOSTNAME:-}}\\\"}\" >/dev/null 2>&1 || true; }",
        "emit_control_heartbeat() { control_plane_request_json /internal/workers/heartbeat \"{\\\"run_id\\\":\\\"${PEETS_CONTROL_RUN_ID}\\\",\\\"worker_id\\\":\\\"${PEETS_CONTROL_WORKER_ID}\\\",\\\"tunnel_session_id\\\":\\\"${PEETS_TUNNEL_SESSION_ID}\\\",\\\"observed_node\\\":\\\"${SLURMD_NODENAME:-${HOSTNAME:-}}\\\"}\" >/dev/null 2>&1 || true; }",
        "emit_control_degraded() { stage=\"$1\"; reason=\"$2\"; control_plane_request_json /internal/workers/degraded \"{\\\"run_id\\\":\\\"${PEETS_CONTROL_RUN_ID}\\\",\\\"worker_id\\\":\\\"${PEETS_CONTROL_WORKER_ID}\\\",\\\"tunnel_session_id\\\":\\\"${PEETS_TUNNEL_SESSION_ID}\\\",\\\"stage\\\":\\\"${stage}\\\",\\\"reason\\\":\\\"${reason}\\\",\\\"observed_node\\\":\\\"${SLURMD_NODENAME:-${HOSTNAME:-}}\\\"}\" >/dev/null 2>&1 || true; }",
        "emit_worker_snapshot() { active_slots=\"${1:-0}\"; idle_slots=\"${2:-0}\"; target_slots=\"${3:-0}\"; memory_pressure_pct=\"${4:-0}\"; memory_gate_open=\"${5:-1}\"; control_plane_request_json /internal/resources/worker \"{\\\"run_id\\\":\\\"${PEETS_CONTROL_RUN_ID}\\\",\\\"worker_id\\\":\\\"${PEETS_CONTROL_WORKER_ID}\\\",\\\"host\\\":\\\"${SLURMD_NODENAME:-${HOSTNAME:-}}\\\",\\\"slurm_job_id\\\":\\\"${SLURM_JOB_ID:-}\\\",\\\"configured_slots\\\":${max_parallel},\\\"active_slots\\\":${active_slots},\\\"idle_slots\\\":${idle_slots},\\\"target_slots\\\":${target_slots},\\\"memory_pressure_pct\\\":${memory_pressure_pct},\\\"memory_gate_open\\\":${memory_gate_open},\\\"queued_slots_inside_worker\\\":0}\" >/dev/null 2>&1 || true; }",
        "emit_slot_snapshot() { slot_id=\"$1\"; slot_state=\"$2\"; artifact_bytes=\"$3\"; control_plane_request_json /internal/resources/slot \"{\\\"run_id\\\":\\\"${PEETS_CONTROL_RUN_ID}\\\",\\\"worker_id\\\":\\\"${PEETS_CONTROL_WORKER_ID}\\\",\\\"slot_id\\\":\\\"${slot_id}\\\",\\\"host\\\":\\\"${SLURMD_NODENAME:-${HOSTNAME:-}}\\\",\\\"allocated_mem_mb\\\":" + str(slot_allocated_mem_mb) + ",\\\"artifact_bytes\\\":${artifact_bytes:-0},\\\"state\\\":\\\"${slot_state}\\\"}\" >/dev/null 2>&1 || true; }",
        "heartbeat_loop() { while sleep \"$PEETS_CONTROL_HEARTBEAT_INTERVAL\"; do emit_control_heartbeat; active_now=$(count_case_jobs); read -r target_now pressure_now gate_open_now avail_mem_now used_mem_now <<EOF",
        "  $(compute_target_slots \"$active_now\")",
        "EOF",
        "idle_now=$(( target_now - active_now )); if [ \"$idle_now\" -lt 0 ]; then idle_now=0; fi; emit_worker_snapshot \"$active_now\" \"$idle_now\" \"$target_now\" \"$pressure_now\" \"$gate_open_now\"; done; }",
        "request_lease() {",
        "  local control_python response",
        "  control_python=\"$(control_python_bin)\"; if [ -z \"$control_python\" ]; then return 1; fi",
        "  response=$(CONTROL_API_URL=\"$PEETS_CONTROL_API_URL\" CONTROL_RUN_ID=\"$PEETS_CONTROL_RUN_ID\" CONTROL_WORKER_ID=\"$PEETS_CONTROL_WORKER_ID\" CONTROL_ACCOUNT_ID=\"${PEETS_ACCOUNT_ID:-account_01}\" CONTROL_SLURM_JOB_ID=\"${SLURM_JOB_ID:-}\" \"$control_python\" - <<'PY'",
        "import json, os, urllib.error, urllib.request",
        "payload = {'run_id': os.environ.get('CONTROL_RUN_ID', ''), 'worker_id': os.environ.get('CONTROL_WORKER_ID', ''), 'job_id': os.environ.get('CONTROL_WORKER_ID', ''), 'account_id': os.environ.get('CONTROL_ACCOUNT_ID', ''), 'slurm_job_id': os.environ.get('CONTROL_SLURM_JOB_ID', '')}",
        "req = urllib.request.Request(os.environ['CONTROL_API_URL'] + '/internal/leases/request', data=json.dumps(payload).encode('utf-8'), headers={'Content-Type': 'application/json'}, method='POST')",
        "try:",
        "    with urllib.request.urlopen(req, timeout=30) as resp:",
        "        print(resp.read().decode('utf-8'))",
        "except urllib.error.HTTPError as exc:",
        ...
        "if not payload.get('lease_available') or not payload.get('lease_token'):",
        "    print('')",
        "else:",
        "    print('\\t'.join([str(payload.get('lease_token') or ''), str(payload.get('slot_id') or ''), str(payload.get('input_name') or ''), str(payload.get('input_relpath') or ''), str(payload.get('output_relpath') or ''), str(payload.get('storage_mode') or 'legacy_payload')]))",
        "PY",
        "}",
        "lease_control_request() {",
        "  endpoint=\"$1\"; lease_token=\"$2\"; slot_state=\"${3:-}\"; reason=\"${4:-}\"; output_relpath=\"${5:-}\"; output_materialized=\"${6:-false}\"",
        "  local control_python response; control_python=\"$(control_python_bin)\"; if [ -z \"$control_python\" ]; then return 1; fi",
        "  response=$(CONTROL_API_URL=\"$PEETS_CONTROL_API_URL\" CONTROL_ENDPOINT=\"$endpoint\" CONTROL_RUN_ID=\"$PEETS_CONTROL_RUN_ID\" CONTROL_LEASE_TOKEN=\"$lease_token\" CONTROL_SLOT_STATE=\"$slot_state\" CONTROL_REASON=\"$reason\" CONTROL_OUTPUT_RELPATH=\"$output_relpath\" CONTROL_OUTPUT_MATERIALIZED=\"$output_materialized\" \"$control_python\" - <<'PY'",
        "import json, os, urllib.error, urllib.request",
        "payload = {'run_id': os.environ.get('CONTROL_RUN_ID', ''), 'lease_token': os.environ.get('CONTROL_LEASE_TOKEN', '')}",
        "if os.environ.get('CONTROL_SLOT_STATE', ''): payload['slot_state'] = os.environ['CONTROL_SLOT_STATE']",
        "if os.environ.get('CONTROL_REASON', ''): payload['reason'] = os.environ['CONTROL_REASON']",
        "if os.environ.get('CONTROL_OUTPUT_RELPATH', ''): payload['output_relpath'] = os.environ['CONTROL_OUTPUT_RELPATH']",
        "if os.environ.get('CONTROL_OUTPUT_MATERIALIZED', '').lower() == 'true': payload['output_materialized'] = True",
        "req = urllib.request.Request(os.environ['CONTROL_API_URL'] + os.environ['CONTROL_ENDPOINT'], data=json.dumps(payload).encode('utf-8'), headers={'Content-Type': 'application/json'}, method='POST')",
        "try:",
        "    with urllib.request.urlopen(req, timeout=30) as resp:",
        "        print(resp.read().decode('utf-8'))",
        "except urllib.error.HTTPError as exc:",
        ...
        "  printf '%s\\n' \"$response\"",
        "}",
        "write_container_bootstrap_script() {",
        "  cat > \"$workdir/container_bootstrap.sh\" <<'EOS'",
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "export PEETS_WORKSPACE_REMOTE=" + _double_quoted_shell_value(workspace_remote),
        "export PEETS_WORKSPACE_MOUNT_ROOT=" + _double_quoted_shell_value(workspace_mount_root),
        "mkdir -p \"$PEETS_WORKSPACE_MOUNT_ROOT\" /work/session_logs /workspace /work/slots /etc/peetsfea_ssh",
        ...
        "EOS",
        "  chmod +x \"$workdir/container_bootstrap.sh\"",
        "}",
        "start_worker_container() {",
        "  if [ \"$PEETS_REMOTE_CONTAINER_RUNTIME\" != \"enroot\" ]; then echo \"[ERROR] pull worker payload requires enroot\" >&2; return 127; fi",
        "  container_name=\"peets-${SLURM_JOB_ID:-nojob}-${PEETS_CONTROL_WORKER_ID}-worker-$$\"",
        "  enroot_base=\"$JOB_TMPFS_ROOT/enroot/${SLURM_JOB_ID:-nojob}/worker-$$\"",
        ...
        "  write_container_bootstrap_script",
        "  if command -v flock >/dev/null 2>&1; then",
        "    ( flock 9; ENROOT_RUNTIME_PATH=\"$enroot_base/runtime\" ENROOT_CACHE_PATH=\"$enroot_base/cache\" ENROOT_DATA_PATH=\"$enroot_base/data\" ENROOT_TEMP_PATH=\"$enroot_base/tmp\" enroot create -f -n \"$container_name\" \"$REMOTE_CONTAINER_IMAGE\" >\"$workdir/enroot.create.stdout\" 2>\"$workdir/enroot.create.stderr\" ) 9>\"$ENROOT_CREATE_LOCK\"",
        "  else",
        "    ENROOT_RUNTIME_PATH=\"$enroot_base/runtime\" ENROOT_CACHE_PATH=\"$enroot_base/cache\" ENROOT_DATA_PATH=\"$enroot_base/data\" ENROOT_TEMP_PATH=\"$enroot_base/tmp\" enroot create -f -n \"$container_name\" \"$REMOTE_CONTAINER_IMAGE\" >\"$workdir/enroot.create.stdout\" 2>\"$workdir/enroot.create.stderr\"",
        "  fi",
        "  rm -f \"$workdir/container.ready\"",
        "  local ssh_mount_dir=\"$workdir/container_ssh\"",
        "  mkdir -p \"$ssh_mount_dir\"",
        "  if [ -z \"$PEETS_CONTROL_SSH_IDENTITY\" ]; then echo \"[ERROR] PEETS_CONTROL_SSH_IDENTITY is required for sshfs workspace mount\" >&2; return 1; fi",
        "  cp -f \"$PEETS_CONTROL_SSH_IDENTITY\" \"$ssh_mount_dir/id_control\"",
        "  chmod 700 \"$ssh_mount_dir\"",
        "  chmod 600 \"$ssh_mount_dir/id_control\"",
        "  local -a container_base_mounts=(--mount \"$REMOTE_HOST_ANSYS_ROOT:/mnt/AnsysEM\" --mount \"$REMOTE_HOST_ANSYS_BASE:/ansys_inc/v252\" --mount \"$REMOTE_HOST_ANSYS_BASE/licensingclient:/mnt/licensingclient\" --mount \"$workdir:/work\" --mount \"$JOB_TMPFS_ROOT:/job_tmpfs\" --mount \"$JOB_DISK_ROOT:/job_disk\" --mount \"/dev/fuse:/dev/fuse\")",
        "  local -a container_runtime_mounts=(\"${container_base_mounts[@]}\" --mount \"$ssh_mount_dir:/etc/peetsfea_ssh\")",
        "  local -a container_prep_cmd=(/bin/bash -lc 'mkdir -p /etc/peetsfea_ssh /workspace /work/slots')",
        ...
        "    enroot start --root --rw \"${container_base_mounts[@]}\" \"$container_name\" \"${container_prep_cmd[@]}\" > \"$workdir/container.prep.stdout\" 2> \"$workdir/container.prep.stderr\"",
        "  ENROOT_RUNTIME_PATH=\"$enroot_base/runtime\" ENROOT_CACHE_PATH=\"$enroot_base/cache\" ENROOT_DATA_PATH=\"$enroot_base/data\" ENROOT_TEMP_PATH=\"$enroot_base/tmp\" \\",
        "    enroot start --root --rw \"${container_runtime_mounts[@]}\" \"$container_name\" /bin/bash /work/container_bootstrap.sh > \"$workdir/container.stdout\" 2> \"$workdir/container.stderr\" &",
        "  container_pid=$!",
        "  for _wait in $(seq 1 180); do",
        "    if [ -f \"$workdir/container.ready\" ] && kill -0 \"$container_pid\" >/dev/null 2>&1; then return 0; fi",
        "    if ! kill -0 \"$container_pid\" >/dev/null 2>&1; then break; fi",
        "    sleep 1",
        "  done",
        "  return 1",
        "}",
        ...
        "  if [ -n \"${container_name:-}\" ]; then ENROOT_RUNTIME_PATH=\"${enroot_base:-}/runtime\" ENROOT_CACHE_PATH=\"${enroot_base:-}/cache\" ENROOT_DATA_PATH=\"${enroot_base:-}/data\" ENROOT_TEMP_PATH=\"${enroot_base:-}/tmp\" enroot remove -f \"$container_name\" >/dev/null 2>&1 || true; fi",
        "  if [ -n \"${enroot_base:-}\" ]; then rm -rf \"$enroot_base\" >/dev/null 2>&1 || true; fi",
        "}",
        "copy_runtime_debug_back() {",
        "  local debug_dest=\"${REMOTE_JOB_DIR:-}\"",
        "  if [ -z \"$debug_dest\" ] || [ ! -d \"$workdir\" ]; then return 0; fi",
        "  mkdir -p \"$debug_dest\" >/dev/null 2>&1 || true",
        "  shopt -s nullglob",
        "  local path rel rel_dir",
        "  for path in \"$workdir\"/launch_probe.txt \"$workdir\"/container*.stdout \"$workdir\"/container*.stderr \"$workdir\"/enroot.create.stdout \"$workdir\"/enroot.create.stderr; do",
        "    [ -f \"$path\" ] && cp -f \"$path\" \"$debug_dest/$(basename \"$path\")\" >/dev/null 2>&1 || true",
        "  done",
        "  for path in \"$workdir\"/slots/*/run.log \"$workdir\"/slots/*/control_plane.log \"$workdir\"/slots/*/exit.code; do",
        "    [ -f \"$path\" ] || continue",
        "    rel=\"${path#$workdir/}\"",
        "    rel_dir=\"$(dirname \"$rel\")\"",
        "    mkdir -p \"$debug_dest/$rel_dir\" >/dev/null 2>&1 || true",
        "    cp -f \"$path\" \"$debug_dest/$rel\" >/dev/null 2>&1 || true",
        "  done",
        "}",
        "prepare_case_dir() { case_dir=\"$1\"; rm -rf \"$case_dir\" >/dev/null 2>&1 || true; mkdir -p \"$case_dir\"; }",
        "write_slot_case_exec_script() {",
        "  case_name=\"$1\"; case_dir=\"$2\"; project_path=\"$3\"; output_dir=\"$4\"",
        "  cat > \"$case_dir/session_case_exec.sh\" <<EOS",
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "case_ram_root=\"/job_tmpfs/${case_name}\"",
        "case_disk_root=\"/job_disk/${case_name}\"",
        "container_case_dir=\"/work/slots/${case_name}\"",
        "mkdir -p \"\\$case_ram_root/tmp\" \"\\$case_ram_root/ansys_tmp\" \"\\$case_disk_root/home\" \"\\$case_disk_root/tmp\" \"\\$case_disk_root/ansys_tmp\" \"$output_dir\"",
        ...
        "export PEETS_SLOT_CORES=" + str(config.cores_per_slot),
        "export PEETS_SLOT_TASKS=" + str(tasks_per_slot),
        "cleanup_slot_processes() {",
        "  cleanup_log=\"\\$container_case_dir/process_cleanup.log\"",
        "  target_work=\"\\$container_case_dir\"",
        "  target_disk=\"\\$case_disk_root\"",
        "  target_tmpfs=\"\\$case_ram_root\"",
        "  { echo \"[cleanup] start $(date '+%Y-%m-%d %H:%M:%S %Z')\"; echo \"[cleanup] target_work=\\$target_work\"; echo \"[cleanup] target_disk=\\$target_disk\"; echo \"[cleanup] target_tmpfs=\\$target_tmpfs\"; } >> \"\\$cleanup_log\" 2>/dev/null || true",
        "  collect_slot_pids() {",
        "    local proc_dir pid comm cmd cwd blob",
        ...
        "      case \"\\$comm \\$cmd\" in",
        "        *ansysedt.exe*|*ansyscl*|*HFSSCOMENGINE.EXE*|*HFSSCOMENGINE*|*hfsscomengine*) ;;",
        "        *) continue ;;",
        "      esac",
        "      blob=\"\\$cwd \\$cmd\"",
        "      case \"\\$blob\" in",
        "        *\"\\$target_work\"*|*\"\\$target_disk\"*|*\"\\$target_tmpfs\"*) printf '%s\\n' \"\\$pid\" ;;",
        "      esac",
        "    done",
        "  }",
        "  pids=\"$(collect_slot_pids | sort -u | tr '\\n' ' ')\"",
        "  if [ -n \"\\$pids\" ]; then echo \"[cleanup] SIGTERM pids=\\$pids\" >> \"\\$cleanup_log\" 2>/dev/null || true; kill \\$pids >/dev/null 2>&1 || true; sleep 5; fi",
        "  for attempt in 1 2 3; do",
        "    pids=\"$(collect_slot_pids | sort -u | tr '\\n' ' ')\"",
        "    [ -n \"\\$pids\" ] || break",
        "    echo \"[cleanup] SIGKILL attempt=\\$attempt pids=\\$pids\" >> \"\\$cleanup_log\" 2>/dev/null || true",
        "    kill -9 \\$pids >/dev/null 2>&1 || true",
        "    sleep 2",
        "  done",
        "  remaining=\"$(collect_slot_pids | sort -u | tr '\\n' ' ')\"",
        "  if [ -n \"\\$remaining\" ]; then echo \"[cleanup] remaining=\\$remaining\" >> \"\\$cleanup_log\" 2>/dev/null || true; else echo \"[cleanup] complete\" >> \"\\$cleanup_log\" 2>/dev/null || true; fi",
        "}",
        "cp -f /work/remote_job.sh.seed \"\\$container_case_dir/remote_job.sh\"",
        "cp -f \"$project_path\" \"\\$container_case_dir/project.aedt\"",
        "cd \"\\$container_case_dir\"",
        "slot_rc=0",
        "bash ./remote_job.sh || slot_rc=\\$?",
        "cleanup_slot_processes || true",
        "cp -a \"\\$container_case_dir/.\" \"$output_dir/\"",
        "exit \"\\$slot_rc\"",
        "EOS",
        "  chmod +x \"$case_dir/session_case_exec.sh\"",
        "}",
        "run_case_in_worker_container() {",
        "  case_name=\"$1\"; case_dir=\"$2\"; project_path=\"$3\"; output_dir=\"$4\"",
        "  write_slot_case_exec_script \"$case_name\" \"$case_dir\" \"$project_path\" \"$output_dir\"",
        "  ENROOT_RUNTIME_PATH=\"$enroot_base/runtime\" ENROOT_CACHE_PATH=\"$enroot_base/cache\" ENROOT_DATA_PATH=\"$enroot_base/data\" ENROOT_TEMP_PATH=\"$enroot_base/tmp\" enroot exec \"$container_pid\" /bin/bash \"/work/slots/$case_name/session_case_exec.sh\"",
        "}",
        "launch_slot() {",
        "  initial_lease_token=\"$1\"; initial_slot_id=\"$2\"; initial_input_name=\"$3\"; initial_input_relpath=\"$4\"; initial_output_relpath=\"$5\"; initial_storage_mode=\"$6\"",
        "  (",
        "    lease_token=\"$initial_lease_token\"; slot_id=\"$initial_slot_id\"; input_name=\"$initial_input_name\"; input_relpath=\"$initial_input_relpath\"; output_relpath=\"$initial_output_relpath\"; storage_mode=\"$initial_storage_mode\"; idle_misses=0",
        "    while true; do",
        "      case_name=\"${slot_id}-${lease_token}\"; case_name=\"${case_name//[^A-Za-z0-9_.-]/_}\"",
        "      case_dir=\"$slots_root/$case_name\"; prepare_case_dir \"$case_dir\"",
        "      if [ \"$storage_mode\" = \"sshfs_direct\" ] && [ -n \"$input_relpath\" ] && [ -n \"$output_relpath\" ]; then",
        "        project_path=\"$PEETS_WORKSPACE_MOUNT_ROOT/$input_relpath\"",
        "        output_dir=\"$PEETS_WORKSPACE_MOUNT_ROOT/$output_relpath\"",
        "      else",
        "        reason=\"unsupported lease storage_mode=${storage_mode:-missing}; single_container_sshfs worker requires sshfs_direct\"",
        "        echo \"[ERROR] $reason\" > \"$case_dir/run.log\"",
        "        lease_control_request /internal/leases/fail \"$lease_token\" \"\" \"$reason\" >/dev/null 2>> \"$case_dir/control_plane.log\" || true",
        ...
        "        slot_hb_pid=''",
        "        slot_heartbeat_loop() { while sleep \"$PEETS_LEASE_HEARTBEAT_INTERVAL\"; do lease_control_request /internal/leases/heartbeat \"$lease_token\" \"RUNNING\" >/dev/null 2>&1 || true; done; }",
        "        lease_control_request /internal/leases/heartbeat \"$lease_token\" \"RUNNING\" >/dev/null 2>&1 || true",
        "        slot_heartbeat_loop &",
        "        slot_hb_pid=$!",
        "        if run_case_in_worker_container \"$case_name\" \"$case_dir\" \"$project_path\" \"$output_dir\" > \"$case_dir/run.log\" 2>&1; then rc=0; else rc=$?; fi",
        "        if [ -n \"$slot_hb_pid\" ]; then kill \"$slot_hb_pid\" >/dev/null 2>&1 || true; wait \"$slot_hb_pid\" >/dev/null 2>&1 || true; fi",
        "        echo \"$rc\" > \"$case_dir/exit.code\"",
        "        artifact_bytes=0",
        "        if [ \"$rc\" -eq 0 ]; then",
        "          emit_slot_snapshot \"$slot_id\" \"SUCCEEDED\" \"$artifact_bytes\"",
        "          lease_control_request /internal/leases/complete \"$lease_token\" \"\" \"\" \"$output_relpath\" true >/dev/null 2>> \"$case_dir/control_plane.log\" || true",
        "        else",
        "          emit_slot_snapshot \"$slot_id\" \"FAILED\" \"$artifact_bytes\"",
        "          failure_reason=$(tail -n 20 \"$case_dir/run.log\" 2>/dev/null | tr '\\n' ' ' | sed 's/[[:space:]]\\+/ /g' | sed 's/\"//g' | sed 's/^ //;s/ $//')",
        "          if [ -z \"$failure_reason\" ]; then failure_reason=\"slot exited rc=${rc}\"; fi",
        "          lease_control_request /internal/leases/fail \"$lease_token\" \"\" \"$failure_reason\" \"$output_relpath\" false >/dev/null 2>> \"$case_dir/control_plane.log\" || true",
        "        fi",
        "      fi",
        "      unset rc",
        ...
        "        idle_misses=0",
        "        IFS=$'\\t' read -r lease_token slot_id input_name input_relpath output_relpath storage_mode <<< \"$next_lease\"",
        "        continue",
        "      fi",
        "      idle_misses=$(( idle_misses + 1 ))",
        "      if [ \"$idle_misses\" -ge 2 ]; then break; fi",
        ...
        "fi",
        "teardown_control_plane() {",
        "  if [ -n \"$heartbeat_pid\" ]; then kill \"$heartbeat_pid\" >/dev/null 2>&1 || true; wait \"$heartbeat_pid\" >/dev/null 2>&1 || true; fi",
        "  stop_control_tunnel",
        "}",
        ...
        "  $(compute_target_slots \"$active_now\")",
        "EOF",
        "  idle_now=$(( target_now - active_now )); if [ \"$idle_now\" -lt 0 ]; then idle_now=0; fi",
        ...
        "      IFS=$'\\t' read -r lease_token slot_id input_name input_relpath output_relpath storage_mode <<< \"$lease_info\"",
        "      launch_slot \"$lease_token\" \"$slot_id\" \"$input_name\" \"$input_relpath\" \"$output_relpath\" \"$storage_mode\"",
        "      continue",
        "    fi",
        "  fi",
        "  if [ \"$active_now\" -gt 0 ]; then wait -n \"${case_pids[@]}\" || true; else sleep \"$PEETS_WORKER_IDLE_POLL\"; fi",
        "done",
    ]
    ...
    return "\n".join(
        [
            ...
            *exclude_lines,
            ...
            "  shopt -s nullglob",
            "  files=()",
            "  for path in launch_probe.txt worker.stdout worker.stderr control_tunnel_bootstrap.err slurm-%j.out slurm-%j.err enroot.create.stdout enroot.create.stderr; do",
            "    [ -e \"$path\" ] && files+=(\"$path\")",
            "  done",
            "  for path in container*.stdout container*.stderr; do",
            "    [ -e \"$path\" ] && files+=(\"$path\")",
            "  done",
            "  if [ \"${#files[@]}\" -eq 0 ]; then",
            "    return 0",
