...
def _build_remote_job_script_content(*, emit_output_variables_csv: bool = True) -> str:
    lines = [
        ...
        "        raise RuntimeError('Output Variables solution data retrieval failed')",
        "    wrote_any = False",
        "    for expression in list(hfss.output_variables):",
        "        _, real_values = solution_data.get_expression_data(expression, formula='real')",
        "        real_value = first_value(real_values)",
        "        if real_value == '':",
        "            continue",
        "        _, imag_values = solution_data.get_expression_data(expression, formula='imag')",
        "        imag_value = first_value(imag_values)",
        "        if has_nonzero_imaginary(imag_value):",
        "            row[f'{expression} (Real)'] = real_value",
        "            row[f'{expression} (Imag)'] = imag_value",
        "        else:",
        "            row[expression] = real_value",
        "        wrote_any = True",
        "    if not wrote_any:",
        "        raise RuntimeError('Output Variables solution data missing')",
        "    write_output_variables_csv(workdir, row)",
        "",
        "",
        "def main() -> None:",
