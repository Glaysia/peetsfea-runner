...
def _build_remote_job_script_content(*, emit_output_variables_csv: bool = True) -> str:
    lines = [
        ...
        "    process = subprocess.Popen(cmd, cwd='/tmp', stdout=stdout_handle, stderr=stderr_handle, env=launch_env)",
        ...
        "    for name, value in dict(app.available_variations.nominal_values).items():",
        ...
        "    design_key = report_slug(getattr(app, 'design_name', '') or 'design')",
        "    reports_dir = workdir / 'all_reports' / design_key",
        ...
        "",
        "",
        "def write_synthetic_input_report(workdir: Path, app: Any) -> Path:",
        "    reports_dir = reports_dir_for_design(workdir, app)",
        ...
        "",
        "",
        "def export_all_reports(app: Any, workdir: Path) -> dict[str, Path]:",
        "    reports_dir = reports_dir_for_design(workdir, app)",
        ...
        "",
        "",
        "def discover_batch_solve_targets(project_file: Path, hfss_design_names: list[str]) -> list[str]:",
        "    project_text = project_file.read_text(encoding='utf-8', errors='ignore')",
        "    solve_targets: list[str] = []",
        "    discovered_setup_map: dict[str, dict[str, list[str]]] = {}",
        "    for design_name in hfss_design_names:",
        "        nominal_setups, optimetrics_setups = extract_design_setup_names(project_text, design_name)",
        "        discovered_setup_map[design_name] = {",
        "            'nominal': nominal_setups,",
        "            'optimetrics': optimetrics_setups,",
        "        }",
        "        for setup_name in nominal_setups:",
        "            solve_targets.append(f'{design_name}:Nominal:{setup_name}')",
        "        for setup_name in optimetrics_setups:",
        "            solve_targets.append(f'{design_name}:Optimetrics:{setup_name}')",
        "    if not solve_targets:",
        "        raise RuntimeError(",
        "            f'No HFSS batch solve targets found in {project_file.name}; '",
        "            f'hfss_design_names={hfss_design_names!r} discovered_setup_map={discovered_setup_map!r}'",
        "        )",
        "    return solve_targets",
        "",
        "",
        "def open_project_for_batch_solve(project_file: Path, grpc_port: int) -> tuple[Desktop, str, str, list[str], list[str]]:",
        ...
        "    solve_targets = discover_batch_solve_targets(project_file, hfss_design_names)",
        "    return desktop, project_name, hfss_design_identifier, discovered_designs, solve_targets",
        "",
        "",
        ...
        "    if normalized_factory == 'MAXWELL3D':",
        ...
        "    for candidate_project_ref in (project_name, str(project_file.resolve())):",
        ...
        "    app = attach_design_app(desktop, project_name, project_file, design_identifier, design_name, 'HFSS', grpc_port)",
        "    if app is None:",
        "        raise RuntimeError(",
        "            f'Failed to bind to open AEDT design project={project_name!r} design={design_name!r}'",
        "        )",
        ...
        "        raise RuntimeError(",
            "            f'Bound AEDT design does not support solve_in_batch project={project_name!r} design={design_name!r} '",
            "            f'app_type={type(app).__name__}'",
        "        )",
        "    return app",
        "",
        "",
        "def wait_for_path(path: Path, *, timeout_seconds: float, process: subprocess.Popen | None = None) -> None:",
        ...
        "                raise RuntimeError(",
        "                    f'Batch solve exited before producing {path.name}; returncode={process_return_code}'",
        "                )",
        "        time.sleep(0.5)",
        "    raise TimeoutError(f'Timed out waiting for {path}')",
        "",
        "",
        "def wait_for_any_path(paths: tuple[Path, ...], *, timeout_seconds: float, process: subprocess.Popen | None = None) -> Path:",
        "    deadline = time.time() + timeout_seconds",
        "    while time.time() < deadline:",
        "        for path in paths:",
        "            if path.exists():",
        ...
        "            if process_return_code is not None:",
        "                raise RuntimeError(",
        "                    f'Batch solve exited before producing any completion marker; '",
        "                    f'returncode={process_return_code} expected={[path.name for path in paths]!r}'",
        "                )",
        "        time.sleep(0.5)",
        "    raise TimeoutError(f'Timed out waiting for any of {[path.name for path in paths]!r}')",
        "",
        "",
        "def run_batch_solve(project_file: Path, solve_targets: list[str], *, cores: int, tasks: int) -> None:",
        "    queue_file = Path(str(project_file) + '.q')",
        "    queue_complete = Path(str(project_file) + '.q.complete')",
        "    queue_completed = Path(str(project_file) + '.q.completed')",
        "    batch_log_path = project_file.parent / 'batch.log'",
        "    if batch_log_path.exists():",
        "        batch_log_path.unlink()",
        "    for index, solve_target in enumerate(solve_targets, start=1):",
        "        for sidecar_path in (queue_file, queue_complete, queue_completed):",
        ...
        "            '-BatchSolve',",
        "            '-machinelist',",
        "            f'list=localhost:{tasks}:{cores}:90%:1',",
        "            '-Monitor',",
        "            solve_target,",
        "            str(project_file.resolve()),",
        "        ]",
        "        batch_env = build_ansys_launch_env()",
        "        with batch_log_path.open('a', encoding='utf-8') as batch_log_handle:",
        "            batch_log_handle.write(f'=== solve_target[{index}]={solve_target} ===\\n')",
        "            batch_log_handle.flush()",
        "            process = subprocess.Popen(",
        "                batch_command,",
        "                cwd=str(project_file.parent),",
        "                env=batch_env,",
        "                stdout=batch_log_handle,",
        "                stderr=subprocess.STDOUT,",
        "            )",
        "            wait_for_path(queue_file, timeout_seconds=120, process=process)",
        "            wait_for_any_path((queue_complete, queue_completed), timeout_seconds=3600, process=process)",
        "            try:",
        "                process_return_code = process.wait(timeout=120)",
        "            except subprocess.TimeoutExpired as exc:",
        "                raise TimeoutError(",
        "                    f'Batch solve completion marker appeared but the solve process did not exit promptly for {solve_target}'",
        "                ) from exc",
        "            if process_return_code != 0:",
        "                raise RuntimeError(",
        "                    f'Batch solve exited with returncode={process_return_code} for solve_target={solve_target!r}'",
        "                )",
        "",
        "",
        "def solve_project_in_batch(project_file: Path, grpc_port: int, *, cores: int, tasks: int) -> None:",
        "    discovery_desktop = None",
        "    project_name = ''",
        "    hfss_design_identifier = ''",
        "    hfss_design_name = ''",
        "    discovered_designs: list[str] = []",
        "    solve_targets: list[str] = []",
        "    try:",
        "        discovery_desktop, project_name, hfss_design_identifier, discovered_designs, solve_targets = open_project_for_batch_solve(project_file, grpc_port)",
        "        hfss_design_name = str(hfss_design_identifier).rsplit(';', 1)[-1].strip() or hfss_design_identifier",
        "        print(",
        "            f'[INFO] batch solve prepared project={project_name!r} '",
        "            f'design={hfss_design_name!r} discovered_designs={discovered_designs!r} solve_targets={solve_targets!r}'",
        "        )",
        "    finally:",
        "        if discovery_desktop is not None:",
        "            try:",
        "                release_desktop_session(discovery_desktop, close_projects=True, close_desktop=False)",
        "            except Exception as exc:",
        "                print(f'[WARN] close discovery desktop failed before batch solve: {exc}')",
        "    run_batch_solve(project_file, solve_targets, cores=cores, tasks=tasks)",
        "",
        "",
        "def reopen_solved_project(project_file: Path, grpc_port: int) -> Desktop:",
        "    desktop = connect_grpc_desktop(grpc_port, close_on_exit=False)",
        ...
        "",
        "",
        "def extract_design_output_row(app: Any, workdir: Path) -> tuple[dict[str, object], list[str]]:",
        "    row: dict[str, object] = {}",
        "    errors: list[str] = []",
        ...
        "    exported_paths: dict[str, Path] = {}",
        "    try:",
        "        canonical_input_report_name = create_input_parameter_report(app)",
        "        canonical_output_report_name = create_output_variables_report(app)",
        "        create_parameter_reports(app)",
        "        update_all_reports(app)",
        "        exported_paths = export_all_reports(app, workdir)",
        "    except Exception as exc:",
        ...
        "        try:",
        "            canonical_input_path = write_synthetic_input_report(workdir, app)",
        "        except Exception as exc:",
        ...
        "            bare_first_row=True,",
        "        )",
        "    if canonical_output_report_name is not None:",
        ...
        "",
        "",
        "def merge_design_row(target_row: dict[str, object], design_name: str, design_row: dict[str, object], *, include_unprefixed: bool) -> None:",
        "    design_key = report_slug(design_name)",
        "    for key, value in design_row.items():",
        "        if include_unprefixed and key not in target_row:",
        "            target_row[key] = value",
        "        target_row[f'{design_key}__{key}'] = value",
        "",
        "",
        "def extract_output_variables_csv(desktop: Desktop, workdir: Path, project_file: Path, grpc_port: int) -> None:",
        ...
        "    }",
        "    (workdir / 'all_reports').mkdir(parents=True, exist_ok=True)",
        "    errors: list[str] = []",
        "    design_results: list[tuple[str, bool, dict[str, object], list[str]]] = []",
        "    for design_name, app in iter_project_design_apps(desktop, project_file, grpc_port):",
        "        try:",
        "            design_row, design_errors = extract_design_output_row(app, workdir)",
        "            design_results.append((design_name, isinstance(app, Hfss), design_row, design_errors))",
        "        finally:",
        "            try:",
        "                release_desktop_session(app, close_projects=False, close_desktop=False)",
        "            except Exception:",
        "                pass",
        "    primary_index = -1",
        "    for index, (_design_name, is_hfss, design_row, _design_errors) in enumerate(design_results):",
        "        if is_hfss and design_row:",
        "            primary_index = index",
        "            break",
        "    if primary_index < 0:",
        "        for index, (_design_name, _is_hfss, design_row, _design_errors) in enumerate(design_results):",
        "            if design_row:",
        "                primary_index = index",
        "                break",
        "    for index, (design_name, _is_hfss, design_row, design_errors) in enumerate(design_results):",
        "        if design_row:",
        "            merge_design_row(row, design_name, design_row, include_unprefixed=(index == primary_index))",
        "        errors.extend(f'{design_name}: {message}' for message in design_errors)",
        "    if len(row) <= 3 and not design_results:",
        "        errors.append('No compatible design session was available for export')",
        ...
        "    solve_desktop = None",
        "    reopened_desktop = None",
        "    try:",
        "        with use_ansys_launch_env():",
        "            solve_project_in_batch(project_file, grpc_port, cores=cores, tasks=tasks)",
        "        try:",
        "            reopened_desktop = reopen_solved_project(project_file, grpc_port)",
        "            try:",
        "                reopened_project = reopened_desktop.odesktop.GetActiveProject()",
        "                if reopened_project is not None:",
        "                    reopened_project.Save()",
        "            except Exception as exc:",
        "                print(f'[WARN] save_project failed but solve completed: {exc}')",
        "            extract_output_variables_csv(reopened_desktop, workdir, project_file, grpc_port)",
        ...
        "        if reopened_desktop is not None:",
        "            release_desktop_session(reopened_desktop, close_projects=True, close_desktop=False)",
...
def _build_worker_payload_script_content(
    *,
    ...
    payload_lines = [
        ...
        "    return 0",
        "  fi",
        "  mkdir -p \"$REMOTE_JOB_DIR/$case_dir\"",
        "  (",
        "    cd \"$case_dir\"",
        "    shopt -s nullglob",
        "    files=()",
        "    for path in run.log exit.code batch.log output_variables.csv output_variables.error.log runtime_logs.json ansys_grpc.stdout.log ansys_grpc.stderr.log pyaedt*.log remote_job.sh run_sim.py project.aedt *.q *.q.complete *.q.completed all_reports project.aedtresults; do",
        "      [ -e \"$path\" ] && files+=(\"$path\")",
        "    done",
        "    if [ \"${#files[@]}\" -eq 0 ]; then",
        "      exit 0",
    ...
    payload_lines.extend(
        [
            ...
            "    periodic_case_sync() {",
            "      while sleep 5; do",
            "        sync_case_artifacts_back \"$case_dir\"",
            "      done",
            "    }",
