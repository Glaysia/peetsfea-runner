...
def _build_pull_worker_payload_script_content(
    *,
    ...
    payload_lines = [
        ...
        "    slot_heartbeat_loop &",
        "    slot_hb_pid=$!",
        f"    if PEETS_SLOT_CORES={config.cores_per_slot} PEETS_SLOT_TASKS={tasks_per_slot} run_case_command \"$case_dir\" > run.log 2>&1; then",
        "      rc=0",
        "    else",
        "      rc=$?",
        "    fi",
        ...
        "        launch_slot \"$lease_token\" \"$slot_id\" \"$case_dir\"",
        "        continue",
        "      fi",
