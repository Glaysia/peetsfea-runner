...
def _build_remote_job_script_content(*, emit_output_variables_csv: bool = True) -> str:
    lines = [
        ...
        "",
        "",
        "def export_all_reports(app: Any, workdir: Path, design_name: str) -> dict[str, Path]:",
        "    reports_dir = reports_dir_for_design_name(workdir, design_name)",
        ...
        "",
        "",
        "def count_design_report_files(workdir: Path, design_name: str) -> int:",
        "    reports_dir = reports_dir_for_design_name(workdir, design_name)",
        "    return sum(1 for path in reports_dir.glob('*.csv') if path.is_file())",
        "",
        "",
        ...
        "    ordered_columns = ['design_name', 'reports_dir', 'report_count', 'error_log']",
        ...
        "                    raise RuntimeError(",
        "                        f'Bound AEDT design does not support solve_in_batch '",
        ...
        "            if not hasattr(app, 'solve_in_batch'):",
        "                raise RuntimeError(",
        "                    f'Bound AEDT session does not support solve_in_batch '",
        "                    f'project={project_name!r} design={design_name!r} app_type={type(app).__name__}'",
        "                )",
        "            return app",
        "        last_error = RuntimeError(",
        ...
        "",
        "",
        "def extract_design_output_row(design_name: str, app: Any, workdir: Path) -> tuple[int, list[str]]:",
        "    errors: list[str] = []",
        "    try:",
        ...
        "    try:",
        "        export_all_reports(app, workdir, design_name)",
        "    except Exception as exc:",
        ...
        "        try:",
        "            report_count, design_errors = extract_design_output_row(design_name, app, workdir)",
        "        finally:",
        "            try:",
        "                release_desktop_session(app, close_projects=False, close_desktop=False)",
        "            except Exception:",
        ...
        "        errors.append('No compatible design session was available for report export')",
        "    if not any(int(row.get('report_count', '0') or '0') > 0 for row in design_output_manifest_rows):",
        "        errors.append('No design report CSV files were exported')",
        "    if errors:",
        ...
        "            close_project = getattr(solve_hfss, 'close_project', None)",
        "            if callable(close_project):",
        "                try:",
        "                    close_project(save=False)",
        "                except TypeError:",
        "                    close_project()",
        "                remove_lock_files(workdir)",
        ...
        "            try:",
        "                remove_lock_files(workdir)",
        "                reopened_desktop = reopen_solved_project(project_file, grpc_port, solve_project_name)",
        "                extract_design_reports(reopened_desktop, workdir, project_file, grpc_port)",
