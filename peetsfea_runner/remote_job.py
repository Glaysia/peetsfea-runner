...
def _build_pull_worker_payload_script_content(
    *,
    ...
    new_payload_lines = [
        ...
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "mkdir -p \"$PEETS_WORKSPACE_MOUNT_ROOT\" /work/session_logs",
        "test -f /root/.ssh/id_control || { echo \"[ERROR] missing container SSH identity: /root/.ssh/id_control\" >&2; exit 1; }",
        "chmod 700 /root/.ssh >/dev/null 2>&1 || true",
        "chmod 600 /root/.ssh/id_control >/dev/null 2>&1 || true",
        "sshfs -o reconnect,ServerAliveInterval=15,ServerAliveCountMax=3,idmap=user,uid=0,gid=0,umask=000,StrictHostKeyChecking=no,UserKnownHostsFile=/dev/null,IdentityFile=/root/.ssh/id_control \"$PEETS_WORKSPACE_REMOTE\" \"$PEETS_WORKSPACE_MOUNT_ROOT\"",
        ...
        "  local -a container_mounts=(--mount \"$REMOTE_HOST_ANSYS_ROOT:/mnt/AnsysEM\" --mount \"$REMOTE_HOST_ANSYS_BASE:/ansys_inc/v252\" --mount \"$REMOTE_HOST_ANSYS_BASE/licensingclient:/mnt/licensingclient\" --mount \"$workdir:/work\" --mount \"$JOB_TMPFS_ROOT:/job_tmpfs\" --mount \"$JOB_DISK_ROOT:/job_disk\" --mount \"/dev/fuse:/dev/fuse\" --mount \"$ssh_mount_dir:/root/.ssh:ro\")",
        ...
        "    enroot start --root --rw \"${container_mounts[@]}\" \"$container_name\" /bin/bash /work/container_bootstrap.sh > \"$workdir/container.stdout\" 2> \"$workdir/container.stderr\" &",
        "  container_pid=$!",
        ...
        "cp -f /work/remote_job.sh.seed \"$case_dir/remote_job.sh\"",
        "cp -f \"$project_path\" \"$case_dir/project.aedt\"",
        "cd \"$case_dir\"",
        "bash ./remote_job.sh",
        "cp -a \"$case_dir/.\" \"$output_dir/\"",
        "EOS",
        ...
        "        artifact_bytes=$(du -sb \"$output_dir\" 2>/dev/null | awk '{print $1+0}')",
        "        if [ \"$rc\" -eq 0 ]; then",
