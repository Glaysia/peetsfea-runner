...
### "컨테이너 부하" 탭 부하 착시 — 실제는 만재인데 작아 보임 (2026-06-16 진단, 미수정·문서화)
**증상:** "컨테이너 부하" 탭이 노드 대부분을 **부하 없음/매우 작음**으로 표시. 그러나 노드에 직접 접속해
확인한 실측은 정반대 — **거의 만재 + 정상 램프업 중**이었다. 표시만 잘못된 **착시**이며, 시뮬 자체는 건강.

**실측(2026-06-16, `srun --overlap`로 잡별 노드 직접 확인):**
- loadavg가 작지 않다: n115 **38.5**, n003 23.7, n055 23.2, n001 22.7 (램프업 정도에 따라 다름).
- AEDT가 **정확히 3분(`EDT_WORKER_STAGGER_SECONDS=180`) 간격으로 1개씩** 기동(n115 가동 21분에 21·18·15·12·9·6·3·0분
  된 ansysedt 8개) → **설계대로 순차 램프업**(슬롯 11개까지 ~33분). 오래 뜬 n001은 풀가동(상위 프로세스 57% CPU = 실제 매트릭스 solve). D-state 0 = I/O 적체 없음.
- 결론: **부하는 작지 않다.** 대시보드 표시가 두 가지 이유로 작아 보이게 만든다(아래 A·B).

**원인 A — `scontrol` 노드 파싱이 노드 대부분에서 실패해 `None`:**
`edt_resources._REMOTE`의 NODES 추출이 `scontrol show node`를 `tr '\n' ' '` 후 **하나의 순서고정 정규식**
(`CPUAlloc…CPUTot…CPULoad…RealMemory…AllocMem…FreeMem`)으로 뽑는다. 라이브에선 n001만 값이 나오고 나머지는
전부 `None`(대시보드 `load=None/None`)으로 떨어졌다. → **수정 방향:** 순서고정 거대 정규식 대신 **필드별 독립 추출**
(`scontrol show node -o` 한 줄 + `grep -oP 'KEY=\K[^ ]+'`)로 바꾸면 견고. 2026-06-16 라이브 검증에서 이 방식은
**RUNNING 노드 6개 전부 정상 파싱**(n001/n003/n055/n085/n114/n115). `parse_remote`의 NODES 컬럼 순서
(`node|load|cpualloc|cputot|freemem|realmem`)는 그대로 두고 **원격 스크립트만** 교체하면 됨.

**원인 B — 분모 착시(더 본질적): `CPUTot`은 물리 노드 전체, 우리 잡 할당이 아님.**
파싱이 되더라도 대시보드 막대는 `load/cputot`. 그런데 **노드는 공유**이고 `CPUTot`은 물리 노드 코어 수다 —
n114/n115는 **256코어**(`CPUAlloc=156/256`, 즉 타 유저 잡도 함께 돎). 우리 잡은 그 중 **48코어만** 할당.
그래서 `load 38 / 256 ≈ 15%`로 **작아 보이지만**, 실제로는 **48코어 할당에 loadavg 38 ≈ 거의 만재**.
또한 `CPULoad`(scontrol)와 `/proc/loadavg`는 **노드 전역**(네임스페이스 비격리)이라 *우리 컨테이너만의* CPU가 아니다.
→ **수정 방향(택1/병행):**
  1. **분모를 우리 잡 할당으로**: 막대 기준을 노드 `CPUTot`이 아니라 **그 잡의 `cpus`**(squeue `%C`, 이미 스냅샷에 `job.cpus` 있음)로.
     단 `CPULoad`는 노드 전역이라 분자/분모 의미가 어긋남 → 라벨에 "노드 전역 부하(공유 노드)"를 명기하고
     `CPUAlloc/CPUTot`(예: 156/256)도 함께 표기해 **공유 노드임을 분명히**.
  2. **진짜 컨테이너별 CPU**(정확하지만 비용↑): 잡 cgroup 내 ansysedt `%CPU` 합을 주기 샘플(`sstat`/`srun --overlap ps`).
     read-only지만 폴링 부하·복잡도 큼 → 시계열 뷰와 함께 검토.
  3. 최소 개선: 막대 외에 **loadavg 절대값**(38 같은 수치)을 1차 지표로 노출 — 절대값은 분모 무관하게 의미 있음.

**적용 시점:** 위 A·B는 **데몬 내 폴러/대시보드 코드 변경** → `systemctl restart` 필요, 그런데 `ExecStopPost=safe_scancel`이
돌면서 **램프업된 잡 전부 teardown·재기동**(각 ~33분 재램프)된다. 표시 전용 결함이므로 **돌고 있는 동안 재기동 금지**.
다음 자연 재기동(노드 만료/장애 복구) 시점에 함께 반영하거나, 사용자가 정비창을 열 때 적용.
